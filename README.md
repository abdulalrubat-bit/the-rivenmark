...
| `debug.html` | Generated by `tools/build-debug.js` — the game plus a debug overlay. Never hand-edited. |
| `art/` | Generated by `tools/export-art.js` — every forged sprite as PNGs, so the art can be looked at outside the single file. See `art/README.md`. |
...
| `android/` | Gradle + WebView shell. `android/sync-assets.js` bundles `debug.html` into the APK. |
