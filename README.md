...
There are two of them and they are not equals any more. **The Phaser build in
`phaser/` is the game.** It is what the APK carries, what is deployed to the
web, and the only one whose look is maintained.

| | `index.html` (canvas) | `phaser/` |
|---|---|---|
| What it is | the simulation's source of truth | the game |
| Ships | no | APK, and the web build |
| Runs from `file://` | yes, with no build step | no — `npm run serve` |
| On an Adreno 840 | 20fps with its effects already shed | a locked 60 |
| Its UI | frozen | maintained |

Every line of simulation still lives in `index.html`, and
`phaser/tools/extract-core.js` lifts it out into `phaser/src/core/core.js`
mechanically; `npm run verify` then runs the same suites against both and
proves they agree. That is why the canvas build stays, and why it is still the
file you edit to change how the game *behaves*. It is not the file you edit to
change how the game *looks*.

**Its presentation is deliberately no longer kept in step.** The two HUDs have
diverged — the Phaser one has the pause screen, the reworked boss bar, the
plated kit and the forged stations, and the canvas one has none of them — and
that is the intended state rather than a job outstanding. Chasing parity across
two renderers costs a second implementation of every UI decision and buys
nothing a player ever sees. If a change touches the HUD, the menus or the
screens, it belongs in `phaser/src/` alone.

The canvas build's own UI is kept only because thirty of the canvas suites
drive it, and because a single file that opens from `file://` with no toolchain
is the fastest way there is to reproduce a simulation bug.
...
| Path | What it is |
|---|---|
| `phaser/` | **The game.** The Phaser build: `src/` is the port, `src/core/core.js` is generated from `index.html`, `public/` is what is served. `phaser/README.md` has its own commands. |
| `index.html` | The canvas build — the simulation's source of truth, and what the canvas suites drive. See *Two builds, one game*. |
| `debug.html` | Generated by `tools/build-debug.js` — the canvas build plus a debug overlay. Never hand-edited. |
| `art/` | Generated by `tools/export-art.js` — every forged sprite as PNGs, so the art can be looked at outside the file it is drawn in. See `art/README.md`. |
...
| `assets/` | Source sheets, cut into `index.html` by `tools/build-art.py`. **Licence unconfirmed** — see below. |
| `fonts/` | The licence for the embedded display face. |
| `android/` | Gradle + WebView shell. `android/sync-assets.js` runs the Phaser deploy and bundles THAT into the APK — it used to bundle `debug.html`, and the script says why it stopped. |
...
|---|---|---|
| `debug.html` | `tools/build-debug.js` | `tools/run-suites.js`, `tools/smoke-profile.cjs` |
| `phaser/src/core/core.js`, `phaser/public/host-stubs.js` | `phaser/tools/extract-core.js` | `phaser/tools/build.js` |
| `phaser/public/core.js` | copied from `src/core/` | `phaser/tools/build.js` |
...
**What was wrong.** `extract-core.js` was run by nothing at all: `index.html`
is the source of truth, `build.js` copied the extracted core forward, and the
extraction between them was `npm run core`, by hand. So an edit to the game
that nobody followed with that command left the web build, the APK and all
sixteen smoke suites running the *previous* rules while reporting green
against the new ones. `deploy.js` checked the atlas *existed* rather than
packing it, which cannot catch an atlas older than `art/`. And nothing made
Gradle run `sync-assets.js`, so `assembleRelease` packaged whatever was
sitting in a gitignored directory — which, when this was written, was a
`core.js` still carrying auto-fire the game had already had deleted.
...
- **`art/`** — `tools/export-art.js` needs Chromium to re-forge the sprites
  from `index.html`. It is committed, so a clean clone can pack an atlas
  without it; re-run it by hand when the forge code changes.
- **`index.html`'s cut sheets** — `tools/build-art.py` is an authoring step,
  not a build step.
...
covering weights 400–900, embedded in `index.html` as a data URI because the
game is one document with no network.
...
This is the canvas build's shape, and since the core is lifted out of it
wholesale it is the Phaser build's simulation too. Sectioned in this order:
