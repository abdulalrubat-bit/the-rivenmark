...
|---|---|---|---|
| C01 | A still press held past 200ms does nothing. | held 250ms: 1 swings; held 150ms: 1 | no |
| C02 | A tap while the blade is still coming round is lost, not queued. | second tap 0.1s before ready: 2 swings of 2; at half the 0.62s beat: 1 of 2 (logged as expired) | no |
| C03 | Aiming out near the edge turns into a gather, stops the swings and slows the stride. | gather began at never; 2 swings before it, 0 after; stride x1 | no |
| C04 | Reaching the edge after a long press gathers at once. | one frame after reaching the edge (0.5s into the press): gather 0 | no |
