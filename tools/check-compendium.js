...
(async()=>{
  ...
  const g = await p.evaluate(()=>{
    ...
    return {
      ...
      aegisR:A('aegis').radius, aegisMit:A('aegis').mitigate,
      purgeCd:A('purge').cd, purgeCh:A('purge').channel, purgePct:A('purge').healPct*100,
      guillReach:A('guillotine').reach, vuln:VULN_MULT,
      bladeCharge:BLADE_CHARGE, bladeTension:BLADE_TENSION, guardBreak:GUARD_BREAK*100,
  ...
  const claims = [
    ...
    ['A landed swing builds 1 Charge',g.bladeCharge,1],
    ['...and 14 Tension',             g.bladeTension,14],
    ['A guard breaks at 55% gathered', g.guardBreak, 55],
